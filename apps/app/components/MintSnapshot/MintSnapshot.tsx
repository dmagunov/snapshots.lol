...
export default function MintSnapshot({ snapshot, onClose }: MintSnapshotProps) {
  ...
  const MINT_SCREENSHOT = (
    <MintSnapshotScreenshot
      ...
      onSuccess={({ screenshotUrl }) => {
        snapshot.image = screenshotUrl;
        dispatch({ type: "MINT_SCREENSHOT_SUCCESS" });
      }}
  ...
  const MINT_SCREENSHOT_SUCCESS = (
    <PixelButtonStyled onClick={() => dispatch({ type: "MINT_META" })}>
      Mint
    </PixelButtonStyled>
  );
  ...
  const MINT_META = (
    ...
    >
      <RoadLoader />
      <p>Pushing metadata to IPFS ... Can take a while ...</p>
    </MintSnapshotMeta>
  );
  ...
  const MINT_CONTRACT = (
    <>
      <HandsLoader />
      <p>Awaiting approval of transaction ...</p>
    </>
  );
  ...
  const MINT_SUCCESS = (
    <>
      <CupLoader />
      <p>
        Congratulations! Your NFT has been successfully minted and added to the
        ...
        >
          ...
          {state.receipt?.transactionHash?.slice(
            state.receipt.transactionHash?.length - 4
          )}
        </a>
      </p>
      <p>
        The interactive version of your NFT can be viewed online at{" "}
  ...
  return (
    <>
      ...
      {["MINT_META"].includes(state.type) && <>{MINT_META}</>}
      ...
      {["MINT_CONTRACT", "MINT_CONTRACT_CONFIRM"].includes(state.type) && (
        ...
        >
          <>
            {["MINT_CONTRACT"].includes(state.type) && <>{MINT_CONTRACT}</>}
      ...
      {["MINT_SUCCESS"].includes(state.type) && <>{MINT_SUCCESS}</>}
    </>
  );
}
